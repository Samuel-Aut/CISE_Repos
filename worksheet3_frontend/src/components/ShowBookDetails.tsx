...
function ShowBookDetails() {
  ...
  useEffect(() => {
    fetch(process.env.NEXT_PUBLIC_BACKEND_URL + `/api/books/${id}`) // change
      .then((res) => {
        return res.json();
      })
    ...
    <div>
      ...
      <table className="table table-hover table-dark table-striped table-bordered">
        ...
        <tbody>
              {" "}
          <tr>
                  <th scope="row">1</th>      <td>Title</td>   
              <td>{book.title}</td>    {" "}
          </tr>
              {" "}
          <tr>
                  <th scope="row">2</th>      <td>Author</td>   
              <td>{book.author}</td>    {" "}
          </tr>
              {" "}
          <tr>
                  <th scope="row">3</th>      <td>ISBN</td>    
             <td>{book.isbn}</td>    {" "}
          </tr>
              {" "}
          <tr>
                  <th scope="row">4</th>      <td>Publisher</td> 
          ...
          <tr>
            ...
            <td>{book.published_date?.toString()}</td>    {" "}
              ...
              {" "}
          <tr>
                  <th scope="row">6</th>      <td>Description</td>
